    ...
    def login(self, username, password, success_redirect=None,
        ...
        if username in self.db.users:
            ...
            if authenticated:
                ...
                # request.__dict__['user'] = self.current_user
                if success_redirect:
...
class Cork(BaseCork):
    ...
    def _session(self):
        """Get session"""
        # session = request.environ[self.opt["name".upper()]]
        return sessions.session
    
    def _save_session(self):
        # self.opt["store".upper()].save_if_modified(self._session)
        sessions.save()
        
    def _delete_session(self):
        # self.opt["store".upper()].delete(self._session)
        sessions.session.pop('username')
        sessions.save()
