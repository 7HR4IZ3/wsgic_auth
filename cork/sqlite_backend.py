...
#		 li = ((k, v) for (k, ktype), v in zip(table.__columns__[1:], row[1:]))
...
#		 self._column_names = [n for n, t in self.__columns__]
#		 self._key_col_num = 0
...
#		 cc = []
#		 for col_name, col_type in self.__columns__:
#			 if col_type == int:
#				 col_type = 'INTEGER'
#			 elif col_type == str:
#				 col_type = 'TEXT'
...
class Table:
	...
	def __init__(self, model, key_col='', values_col=''):
		self._ = model

		self._column_names = list(self._.__columns__)
		self._key_col_name = key_col
		self._key_col = self._key_col_name
	...
	def __len__(self):
		query = "SELECT count() FROM %s" % self._.__name__
	...
	def __contains__(self, key):
		#FIXME: count()
		query = "SELECT * FROM %s WHERE %s='%s'" % \
			(self._.__class__.__name__, self._key_col, key)
# 		row = db.execute(query)
		row = self._.objects.get(**{self._key_col: key})
...
# 		question_marks = ', '.join('?' for x in col_values)
# 		query = "INSERT OR REPLACE INTO %s (%s) VALUES (%s)" % \
# 			(self._.__class__.__name__, col_names, question_marks)

# 		ret = db.execute(query, args=col_values)
		value[self._key_col] = key
		self._.objects.create(**value)


	def __getitem__(self, key):
		query = "SELECT * FROM %s WHERE %s='%s'" % \
			(self._.__class__.__name__, self._key_col, key)
# 		row = db.execute(query, one=True)
		row = self._.objects.get(**{self._key_col: key})
	...
	def __iter__(self):
		"""Iterate over table index key values"""
		query = "SELECT %s FROM %s" % (self._key_col, self._.__class__.__name__)
# 		result = db.execute(query)
		result = self._.objects.get(self._key_col)
	...
	def iteritems(self):
		"""Iterate over table rows"""
		query = "SELECT * FROM %s" % self._.__class__.__name__
		result = self._.objects.get()
		for row in result:
			d = dict(zip(self._column_names, row))
			d.pop(self._key_col)
			yield (self._key_col, d)
	...
	def pop(self, key):
		d = self.__getitem__(key)
		query = "DELETE FROM %s WHERE %s='%s'" % \
			(self._.__name__, self._key_col, key)
		db.execute(query, one=True)
		#FIXME: check deletion
		return d
...
class SingleValueTable(Table):
	def __init__(self, *args):
		super(SingleValueTable, self).__init__(*args)
		self._value_col = args[2]
		self._key_col = args[1]

	def __setitem__(self, key, value):
		"""Create or update a row"""
		assert not isinstance(value, dict)
		query = "INSERT OR REPLACE INTO %s (%s, %s) VALUES (?, ?)" % \
			(self._.__class__.__name__, self._key_col, self._value_col)
		data = {self._key_col: key, self._value_col: value}
		self._.objects.create(**data)

	def __getitem__(self, key):
		query = "SELECT %s FROM %s WHERE %s='%s'" % \
			(self._value_col, self._.__name__, self._key_col, key)
# 		row = db.execute(query, one=True)
		row = self._.objects.get(self._value_col, **{self._key_col: key})
		if not row:
			return None

		return row[0][self._value_col]

class SQLiteBackend:
	def __init__(self, db_, users, roles, pending, initialize=False):
		global db
		db = db_
		self.users = Table(users, "username")
		self.roles = SingleValueTable(roles, "role", "level")
		self.pending_registrations = Table(pending, "code")
